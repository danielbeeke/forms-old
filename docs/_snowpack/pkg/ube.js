...
const {create: create$1, defineProperties} = Object;
...
const render = (where, what) => {
  ...
  const info = cache$2.get(where) || cache$2.set(where, createCache());
...
const html$1 = augment(html);
const svg$1 = augment(svg);

function reshaped(...args) {
  const [template] = args;
  let known = cache$3.get(template);
  if (known) {
    for (let i = 0, {length} = known; i < length; i++)
      args[known[i].i] = known[i].v;
  }
  else {
    cache$3.set(template, known = []);
    for (let i = 1, {length} = args; i < length; i++) {
      ...
      if (typeof current === 'function' && 'tagName' in current) {
        ...
        switch (prev[prev.length - 1]) {
          case '<':
            current = asStatic(`${tagName} ${UBE}=${indexOf$1(current)}`);
            break;
          case '/':
...
export { html$1 as html, render$1 as render };
